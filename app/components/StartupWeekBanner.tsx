...
export default function StartupWeekBanner() {
  const { showBanner, hideBanner, isLoaded } = useBanner();

  // Return a placeholder with the same height to prevent layout shift
  if (!isLoaded) {
    return <div className="w-full h-[42px] bg-gradient-to-r from-indigo-900 via-purple-900 to-indigo-900"></div>;
  }

  // Use AnimatePresence to handle smooth transitions
  return (
    <AnimatePresence>
      {showBanner && (
        <motion.div 
          className="w-full bg-gradient-to-r from-indigo-900 via-purple-900 to-indigo-900 text-white py-2 px-4 relative overflow-hidden"
